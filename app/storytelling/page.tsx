          ...
          <HeroText delay={0.45}>
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <motion.div whileHover={{ opacity: 0.85 }} whileTap={{ scale: 0.97 }}>
                <Link href="/apply" style={{ fontFamily: BODY, border: '1.5px solid rgba(255,255,255,0.5)', background: 'transparent', color: '#ffffff', padding: '13px 28px', borderRadius: 4, fontSize: 13, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', textAlign: 'center', display: 'block' }} className="hover:border-white transition-all">
                  Apply for Membership →
                </Link>
              </motion.div>
              <motion.div whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.97 }}>
                <a href="#the-series" style={{ fontFamily: BODY, border: '1.5px solid rgba(82,170,252,0.4)', color: 'rgba(255,255,255,0.75)', padding: '13px 28px', borderRadius: 4, fontSize: 13, fontWeight: 300, letterSpacing: '0.04em', textAlign: 'center', display: 'block' }} className="hover:border-white/60 hover:text-white transition-all">
                  The Heroes Teaser ↓
                </a>
              </motion.div>
            </div>
        ...
        <div className="ticker-track">
          {repeated.map((word, i) => (
            <span key={i} className="flex items-center">
              <span style={{ fontFamily: HEADING, color: '#092866', fontSize: 26, fontWeight: 300, letterSpacing: '0.06em', textTransform: 'uppercase' as const, whiteSpace: 'nowrap', padding: '0 24px' }}>
                {word}
              </span>
              <span style={{ color: 'rgba(9,40,102,0.3)', fontSize: 10 }}>·</span>
            </span>
          ))}
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <FadeUp>
          <div className="max-w-3xl mb-12">
            ...
            <h2 style={{ fontFamily: HEADING, color: '#092866', fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 300, lineHeight: 1.1, letterSpacing: '-0.01em', marginBottom: 20 }}>
              Heroes Documentary
            </h2>
        ...
        <ScaleIn delay={0.15}>
          <div style={{ position: 'relative', width: '100%', maxWidth: 960, borderRadius: 4, overflow: 'hidden', aspectRatio: '16/9', backgroundColor: '#0B1220' }} className="mx-auto">
            <Image fill style={{ objectFit: 'cover' }} src="/images/pic.jpeg" alt="Heroes Documentary" />
            <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'rgba(8,15,28,0.35)' }}>
              <a href="https://drive.google.com/file/d/1CdeePbYiqLYS0w-O5x4cq5EzUKpUE4bg/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="absolute inset-0 flex items-center justify-center" style={{ background: 'rgba(8,15,28,0.35)' }}>
                <div style={{ width: 68, height: 68, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.15)', border: '2px solid rgba(255,255,255,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
  ...
  return (
    <section style={{ backgroundColor: NAVY_HERO, position: 'relative', overflow: 'hidden' }} className="py-16 md:py-16">
      ...
      <div className="max-w-4xl mx-auto px-6" style={{ position: 'relative', zIndex: 1 }}>
        <FadeIn>
          <div style={{ fontFamily: HEADING, color: '#52aafc', fontSize: 'clamp(64px, 10vw, 110px)', lineHeight: 0.75, fontWeight: 900, marginBottom: 2, userSelect: 'none' as const }} aria-hidden="true">"</div>
        </FadeIn>
        <blockquote>
          <FadeUp delay={0.25}>
            <p style={{ fontFamily: HEADING, color: '#ffffff', fontSize: 'clamp(18px, 3vw, 34px)', fontWeight: 300, fontStyle: 'italic', lineHeight: 1.45, letterSpacing: '-0.01em' }}>
...
const reasons = [
  { title: 'For Athletes', desc: 'Your story is your most durable asset. Long after the competition ends, the person who overcame — and what they did with it — is what people remember and follow.', img: '/images/360_F_781926021_ejEZxJH5oiCtZt0RlVThxUhNRntxxl9o.jpg' },
  { title: 'For Future Generations', desc: "Young athletes don't need highlight reels. They need proof that someone who doubted like them, fell like them — got back up and built something.", img: '/images/Connection.png' },
  { title: 'For the Culture', desc: 'Sport shapes culture. The stories of athletes — told honestly and with depth — elevate what we all believe is possible.', img: '/images/AdobeStock_1974622747.jpeg' },
];
      ...
      <div className="max-w-7xl mx-auto px-6">
        <FadeUp>
          <div>
            ...
            <h2 style={{ fontFamily: HEADING, color: '#092866', fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 300, lineHeight: 1.1, letterSpacing: '-0.01em', marginBottom: 48 }}>
          ...
          {reasons.map(({ title, desc, img }) => (
            <StaggerItem key={title}>
              <MotionDiv whileHover="hover" initial="rest" animate="rest" variants={cardHoverVariants}>
                <div style={{ height: 200, borderRadius: 4, overflow: 'hidden', marginBottom: 20, position: 'relative', borderTop: '1.5px solid #092866' }}>
                  <Image fill style={{ objectFit: 'cover' }} src={img} alt={title} />
                </div>
                <div style={{ padding: '0 12px' }}>
                  <h3 style={{ fontFamily: HEADING, color: NAVY, fontSize: 15, fontWeight: 300, letterSpacing: '0.04em', textTransform: 'uppercase' as const, marginBottom: 12 }}>{title}</h3>
                  <p style={{ fontFamily: BODY, color: '#4A5568', fontSize: 14, fontWeight: 300, lineHeight: 1.8, paddingBottom: 12 }}>{desc}</p>
                </div>
              </MotionDiv>
            </StaggerItem>
          ))}
